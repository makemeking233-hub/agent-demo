...
| `2026-09-22-shadcn-frontend-migration/` | shadcn-frontend-migration §3+§4：前端 CSS Modules 全量迁 Tailwind v4 + shadcn/ui（18 个组件、`.module.css` 21→0）；同时偿还 C1–C7 技术债（form/sonner stub 恢复、`@ts-nocheck` 清零、`noImplicitAny` 收回、axe 纠正误判、hc 主题填全、主题机制单轨化、删 `lib/theme.ts`） | 2026-09-22 | 311 前端用例（41 文件，含 §2/§3 新增 15 个）+ 912 Java 回归（533 core + 379 web） | ✅ **全部全绿**：mvn BUILD SUCCESS（0 jacoco 违规）；vitest 311 passed；tsc **2**（基线内）；vite build ✓ | ✅ | 已归档 |
...
| `2026-09-23-fix-jacoco-rule/` | fix-jacoco-rule 修 agent-core 的 **jacoco 门禁空转**（`BUNDLE` 元素 + 类名通配 includes → 0 个考核对象 → 任何覆盖率都通过）；改为 `PACKAGE` + 根包/子包**成对** includes，并逐包补测 7 个未达标包 | 2026-09-23 | 新增 **74** 例（10 个测试类新增或扩充）；agent-core 658/0 + agent-web 399/0；前端 361 全过；tsc 2 ≤ 7 | ✅ 两模块 `All coverage checks have been met` + BUILD SUCCESS；**抬升实验证明门禁真的会拦**（对照：修复前规则 0.99 仍成功）；11 个包全达标（原 5 根包 + 被发现漏检的 6 子包）。另发现 **`AnthropicProvider` 在真实 SSE 下零产出**（D-03，已固化未修） | ✅ | 已归档 |
...
- **执行要点**：worktree `.worktrees/add-provider-catalog-abstract` 隔离作业（§2.7），承接前一 session 已落地的 13/62 task；本次补完 task 5-12。门禁 1 `mvn -o -pl agent-core,agent-web verify -DskipNpm=true -Dsurefire.excludes=**/e2e/**` → 合并前 agent-core 461/461 + agent-web 258/258 全绿、前端 241/241（25 文件）、tsc 6 个错误；**同步 `main`（门禁 4，合入 63 个并行提交，解 4 处代码冲突 + 1 处 spec 冲突）后重跑**，agent-core **519/519** + agent-web **373/373** = **892 全绿**、前端 **290/290**（38 文件）、tsc 降为 **2** 个错误（**低于**基线 7）。jacoco 由合并前 3 个既有违规收敛为 **1** 个（仅 `web.security: branches 0.63`，与 `main` 一致）；`web.api` / `web.api.catalog` 两个包由违规 → 通过。
- **既有失败归因（门禁 5）**：（1）`web.security` jacoco branch 0.63——合并前后一致，非本次引入；（2）前端 9 个 `ReferenceError: EventSource is not defined` unhandled rejection——来源是 `main` 的 `src/lib/settings-sse.ts`（settings SSE 在 jsdom 中无 EventSource），已在主工作区 `main` 干净 HEAD 上用同两个测试文件复现（2 文件 13 用例通过、2 errors，同类同比例），判定为 `main` 既有问题，记录后放行。详见批次 `test-report.md` §1.1。
...
### 2.16 `2026-09-18-fix-agent-home-isolation/` — agent home 解析收敛与测试隔离
...
### 2.21 `2026-09-22-snip-pairing-repair/` — 修会话恢复裁剪切断 tool_calls 配对组

- **测试目标**：修一个「会话永久不可用」级别的问题——会话 `2026-09-18T13-34-27-b1569b39` 每轮请求都收到上游 400 `Messages with role 'tool' must be a response to a preceding message with 'tool_calls'`。根因是 `SessionResumeLoader.snip` **逐条**从头部裁剪历史，裁剪点可能落在 `assistant(tool_calls)` 与其 `tool_result` **之间**；而反向孤儿修复（原 `SessionResumeLoader.injectOrphanSkeletons`）只在 `toMessages` 内执行，即在 `snip` **之前**，此后无人兜底 → 孤儿 tool 消息进入内存历史，每轮重放都 400。
- **执行要点**：worktree `.worktrees/snip-pairing-repair`（分支 `fix/snip-pairing-repair`）隔离作业（§2.7），走 OpenSpec 四阶段（proposal → design → delta spec → tasks → apply → archive）。TDD 三步各自先红后绿：① `snipDoesNotSplitToolCallGroup` 先红（`expected: <false> but was: <true>`，13 run / 1 fail）→ 实现裁剪点按配对组对齐（`drop` 跳过前导 `ToolResult`）→ 13/13 绿；② `ToolCallPairingTest` 7 个新例先红（test-compile 报「找不到符号 repairOrphanResults」）→ 把反向修复提取为 `ToolCallPairing.repairOrphanResults` 纯函数、连续孤儿合并为一条骨架 → 15/15 绿 + `SessionResumeLoaderTest` 13/13 回归绿；③ `AgentLoopToolPairingTest.requestRepairsBothPairingDirectionsOnDirtyHistory` 先红（反向孤儿原样发出）→ `AgentLoop.toRequest` 改 `repairOrphanResults(repair(history.all()))` → 2/2 绿。门禁：分支 agent-core **545/0** + agent-web **379/0**，合并 `main`（`8b60312`）后重跑同样 545/0 + 379/0，两次均 `BUILD SUCCESS`；前端 vitest `40 files / 302 tests` 全过；`tsc --noEmit` 6 个错误 ≤ 基线 7。
...
### 2.22 `2026-09-22-shadcn-frontend-migration/` — 前端 CSS Modules 全量迁 Tailwind + shadcn/ui

- **测试目标**：验证前端从 **CSS Modules 全量迁移**到 **Tailwind v4 + shadcn/ui** 后——（1）既有组件行为契约不变（props / 回调 / role / aria / testid）；（2）`*.module.css` 全部删除且无 `.tsx` 再引用；（3）主题机制单轨化（`<html data-theme>`）且三套主题（light/dark/hc）变量完整；（4）`strict: true` 完整生效；（5）axe 可自动跑通。
- **执行要点**：worktree `.worktrees/shadcn-components-p2` 隔离作业（§2.7）。分 3 个 batch 迁完 18 个组件（batch C 渲染 8 个 / batch B 设置 4 项（`SettingsRows` 影响 6 个使用方）/ batch A 布局 7 个），每迁一个（或 2–3 个）立即跑该组件测试 + 全量 tsc。最终门禁：`mvn -o -pl agent-core,agent-web verify` → **agent-core 533 + agent-web 379 = 912 全绿、0 jacoco 违规**；前端 `npx vitest run` → **311 passed（41 文件）**；`npx tsc --noEmit` → **2**（基线内）；`npx vite build` → ✓。`.module.css` 计数 **21 → 0**。
- **关键发现**：（1）**最大教训：不要用 esbuild/babel 重新格式化第三方 JSX 输出**——§1 为修 shadcn 4.x 单行输出而用 esbuild transform，其 JSX transform 会保留空白文本节点，把 `<DialogPrimitive.Close asChild><Button/></DialogPrimitive.Close>` 变成 `[" ", <Button/>, " "]` 三个子节点，违反 Radix Slot 的「单个元素子节点」契约，导致 Dialog 在 jsdom/happy-dom 下均渲染失败。当时**误判为「jsdom 不支持 asChild」并写进了 limitation 清单**；本次从 registry 重拉组件后问题消失，连带 `@ts-nocheck`（5 处）、`noImplicitAny`、`form/sonner` stub 三个技术债一并还清。（2）**第二个误判：把「用法错误」当成「环境限制」**——`expect(container).toHaveNoViolations()` 报 `Unexpected aXe results object`，实际是该 matcher **接收 axe 结果对象而非 DOM 元素**；改正为 `const r = await axe(el); expect(r.violations).toEqual([])` 后 axe 在 jsdom 下完全正常，`happy-dom` 依赖随之变成多余（已卸载）。两次误判的共同点是「把『我没用对』快速归因为『它不行』」，而一旦写进 limitation 就再没人回头。（3）**断言 CSS Module 类名 = 把实现细节写进测试**——6 个用例因断言 `rowUser` / `tableWrap` / `workspaceMissing` 失败；改为断言行为（`flex-row-reverse` / `overflow-x-auto`）与语义类（`text-destructive`）。（4）**共享样式表要整体处理**——`SettingsRows.module.css` 被 6 个组件共用，必须一次迁完 6 处再删文件。（5）**子代理空转**——分派的 2 个 subagent 均立即失败（无 commit / 无改动 / 无结束消息），改为自己执行；建议分派前先探活，失败两次即切回。（6）**一次越界**——迁移初期在 `main` 上直接改了代码，发现后立即建分支承载改动 + 主工作区切回 `main` + worktree 隔离；教训是「动手前先 `git status -sb` 确认分支」。
- **deferred（明确未覆盖）**：全量组件级 axe 扫描（本次只给 `ReasoningEffortSelect` 建了 a11y 测试）；Playwright E2E（沙箱无浏览器）；像素级视觉回归（无截图基线）；三主题实机视觉走查（需人工在浏览器切换）。
- **数据隔离**：全部为纯单元测试（Java Mockito mock / 前端 jsdom + mock api），不启真实 Spring 应用、不写 `~/.agent-demo`；**未产生需清理的用户数据**。
- **四件套**：`test-design.md` / `test-cases.md` / `test-report.md` / `test-review.md` ✅
- **附带产物**：`docs/frontend-design.md`（项目级前端规范，design §7 要求）。
- **归档状态**：已归档。
...
### 2.23 `2026-09-23-fix-jacoco-rule/` — 修 agent-core 的 jacoco 门禁空转

- **测试目标**：修一个「测试有效性」问题——`agent-core` 的覆盖率门禁长期**真空通过**（规则写了 LINE≥80% / BRANCH≥70%，但一个考核对象都没有）。因此本批第一目标不是「补覆盖率」，而是**证明门禁真的会拦**；第二目标才是把缺口补到达标。
- **执行要点**：worktree `.worktrees/fix-jacoco-rule`（分支 `fix/jacoco-rule`）隔离作业（§2.7），走 OpenSpec 四阶段。T1 改 pom（`BUNDLE` → `PACKAGE`）并用**阈值抬升实验**验证（把阈值临时改成 `0.99`，真规则应失败）；T2 按 `jacoco.csv` 数据逐包补测 7 个未达标包（74 例）；T3 把规则写进 `testability` spec 并同步 `AGENTS.md`。门禁：agent-core **658/0** + agent-web **399/0**，两模块 `All coverage checks have been met`，`BUILD SUCCESS`；前端 vitest **361 passed / 43 文件**；`tsc --noEmit` 2 ≤ 基线 7（本 change 零前端改动，前端在主工作区跑，源码逐字节相同）。
- **关键发现**：（1）**同一个错误在一个 change 里出现了两层**——`BUNDLE` 元素下 `includes` 过滤的是 bundle 名（类名模式匹配不到 → 0 个考核对象）；改成 `PACKAGE` 后又有第二层：`includes` 过滤的是**包名**，`X` 只中根包、`X.*` 只中子包，**只写其一会静默漏掉另一半**。第一层是本次要修的缺陷，第二层是实施中发现的——当时只写了 5 个根包名，`tools.file`(BRANCH 0.50) / `provider.minimax`(LINE 0.53) 这些最差包不在考核内而构建照样报 `met`。（2）**「验会不会拦」与「验拦了哪些」是两件事**：只做抬升实验会把第二层漏掉；是「违规清单 vs 独立复算 CSV」这一步抓出来的。这条已写进 spec 的 Scenario。（3）**对照组不能省**：只对修好的规则做实验，排除不了「实验恒真」；对**修复前**的规则也做一次得到「仍成功」，两次结果不同才有说服力。（4）**抬升实验有个前置坑**：`jacoco.exec` 不存在时 check 无数据可判会直接跳过（表现为「改了阈值仍成功」）；必须先跑一次 `mvn test` 生成 exec。（5）**`git checkout` 会吃掉未提交改动**：做实验想还原 pom 时执行 `git checkout -- agent-core/pom.xml`，把尚未 `git add` 的 includes 改写一并还原了；改用「原文存变量、实验后写回」。（6）**同 worktree 的 `AGENTS.md` 撞号**：并行 agent 也把版本升到 v0.1.8（worktree 强制隔离），本批的 jacoco 一条改为 **v0.1.9** 并保留对方内容。
- **缺陷清单（本批）**：D-01 门禁真空（**已修**）；D-02 子包漏检（**已修**）；D-03 `AnthropicProvider.streamChat` 在真实 `text/event-stream` 下**零产出**（Spring `ServerSentEventHttpMessageReader` 剥掉 `data: ` 前缀，而 `parseSseLine` 要求该前缀；既有 12 个单测都直接调 `parseSseLine("data: ...")` 绕过了 HTTP 层，所以全绿）——**未修**，已用 characterization 用例固化并建议单开 change；D-04 `agent-web` 的 `com.example.agent.web.*` 漏根包（同类机制、另一模块）——**未修**，已记录。
- **四件套**：`test-design.md` / `test-cases.md` / `test-report.md` / `test-review.md` ✅
- **归档状态**：已归档（OpenSpec change 归档为 `2026-09-22-fix-jacoco-rule`，delta 已并入 `openspec/specs/testability/spec.md`：+1 新需求 `覆盖率门禁`）。注：`openspec archive` 未识别本 change 的 `tasks.md` checkbox（报 `Task status: No tasks`），归档时删除源目录却没搬走该文件，已按实际执行结果重建于归档目录。
