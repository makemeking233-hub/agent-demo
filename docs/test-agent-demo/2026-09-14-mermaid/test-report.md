...
> mermaid 在 jsdom 中**无法真实渲染**——它依赖 `getBBox`、`getComputedTextLength` 等 jsdom 未实现的 SVG 测量 API。所以单测只能 mock 契约，**真能出图必须由浏览器实开验证**。本次按 tasks.md 5.3 实开：

| 用例 | 验证步骤 | 观察 | 结论 |
|------|---------|------|------|
| BRW-01 合法图 | 在对话区粘贴 `flowchart TD; A-->B` 围栏 → 发送 | 对应位置出现 `<svg>` 节点，可滚动，箭头清晰 | ✅ |
| BRW-02 非法语法 | 改成 `flowchart TD` 后直接接中文分号，闭合后发送 | 出现提示行 `mermaid 图渲染失败：...` + 下方原始源码块；同消息内其他段落/代码块正常 | ✅ |
| BRW-03 Network | DevTools → Network 抓包 | 首次出现围栏时才请求 `mermaid.core-*.js` 与对应图型 chunk；无围栏的会话零请求 | ✅ |

> 这是「单测只证契约 + 浏览器证真出图」两层证据的完整闭环。**单测无法替代这一步**。
