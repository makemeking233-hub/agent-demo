...
| 编号 | 用例 | 期望 | 实测 | 落地 |
|:----:|------|------|------|:----:|
| BRW-01 | 发送合法 mermaid 围栏 | SVG 图真的画出来、可滚动 | 见 `test-report.md` §3 浏览器实开记录 | ✅ |
| BRW-02 | 发送语法非法 mermaid 围栏 | 提示行 + 源码块可见，不白屏 | 同上 | ✅ |
| BRW-03 | DevTools Network 观察 mermaid chunk | 仅在首次出现围栏时才请求 | 同上 | ✅ |

> 「真能出图」与「兜底外观」必须由浏览器实开证明，单测只证明契约。详见 `test-report.md` §3。
