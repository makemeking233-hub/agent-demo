...
定义 agent-demo v0.1 Web UI 的用户可见行为契约——浏览器侧 React 前端与 Java Spring WebFlux 后端之间的 HTTP / SSE 协议面、信任模型、in-chat 权限交互，以及与既有 `cli` capability 的边界。本规约涵盖 v0.1 全部对外行为；v0.2 将另行追加 permission UI、session 历史、settings。

> **关于翻译约定**：Requirement 标题、Scenario 标题、说明性文字用中文。**协议契约本身保留英文**——HTTP path、SSE event `type` 字段名、JSON 字段名、`WHEN/THEN/GIVEN` 模板词——这些是前后端共同遵守的机器契约，翻译反而错位。

## ADDED Requirements

> 本节标题保留英文 `ADDED Requirements`（OpenSpec schema 强制要求），下面所有 Requirement / Scenario 标题与说明均为中文。
