...
- [x] 6.1 `frontend/src/api/chat.ts` types 加 `deltaType: "thinking"` 适配 + `Message.thinking: string[]` 字段 + `Message.reasoningTokens` 字段
- [x] 6.2 `useChatStream` 解析 `delta_type="thinking"` 事件 → 累积到当前 assistant 消息的 `thinking` 字段（独立于 text）
- [x] 6.3 `useChatStream` 解析 `usage.reasoning_tokens` → 写 reasoningTokens（前端展示）

## 7. 前端 UI 组件

- [x] 7.1 `frontend/src/components/ThinkingCollapse.tsx` 新增可折叠组件（默认折叠 + 标题"思考过程 (N token)" + 超 2000 token "查看更多"）
- [x] 7.2 `MessageBubble.tsx` 集成 `ThinkingCollapse`：assistant 消息上方显示（如果 thinking 非空）
- [x] 7.3 `ModelsDropdown.tsx` 新增模型选择下拉（从 `GET /api/chat/models` 拉列表 + 调用 `SlashCommand` 切模型）
- [x] 7.4 `TopBar.tsx` 集成 `ModelsDropdown`（放在 Settings 图标旁）
- [x] 7.5 `ChatPanel` / `MessageBubble` 跨 turn thinking 合并（按事件序累加，不按 turn 分割）
- [x] 7.6 `ChatPanel` abort 时 thinking 标记"已中断"（message_stop event finish_reason="aborted"）

## 8. 前端测试

- [x] 8.1 `tests/chat-stream-thinking.test.tsx` 扩 useChatStream 解析 thinking delta + 跨 turn 合并 + abort
- [x] 8.2 `tests/MessageBubble.thinking.test.tsx` 扩 ThinkingCollapse 渲染 / 折叠 / 跨 turn 合并
- [x] 8.3 `tests/ThinkingCollapse.test.tsx` 扩 超 2000 token 截断 + 展开按钮

## 9. 后端测试

- [x] 9.1 `StreamChunkThinkingDeltaTest` 单独测试 ThinkingDelta record + Visitor 派发
- [x] 9.2 `AgentLoopThinkingTest` 扩 thinking 累积 + history 合并 + abort 测试
- [x] 9.3 `ChatStreamServiceThinkingTest` 扩 thinking SSE 透传
- [x] 9.4 `mvn -pl agent-core test` 全绿 + `mvn -pl agent-web test` 全绿（跳过 E2E）
- [x] 9.5 `mvn -pl agent-core,agent-web verify` jacoco 门禁通过（LINE≥80% / BRANCH≥70%）
