# sandbox-policy Specification（rewrite-permission-mode-dsh ADDED）

## Purpose

统一描述 agent-demo 的权限模式（SandboxMode 4 档）+ 跨能力 sandbox policy 服务（SandboxPolicyService）+ 写入根派生（writableRoots）+ 拒绝反馈结构化（FsDenialKind）+ 实时模式切换（escalate）+ TOCTOU 防护。该 capability 合并原 `permission-mode` 的全部需求与 `settings` 中 `permission.mode` 段，统一对齐 dsh web 的 SandboxPolicy 抽象。
