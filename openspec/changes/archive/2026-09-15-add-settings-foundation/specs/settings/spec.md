# settings Capability（delta for M1：add-settings-foundation）

> M1 仅产出 modal shell + 数据底座。具体设置项（外观/权限/Enter 行为）的 REQUIREMENTS 由 M2 (`add-settings-general-items`) 增量补充。
