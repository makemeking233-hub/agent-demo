...
class ContextCompressorCompactTest {
    ...
    void compactCollapsesAndSummarizes() {
        ...
        for (int i = 0; i < 10; i++) hist.append(new Message.User("msg-" + i + " " + "很长的内容 ".repeat(50)));
        ...
        StepVerifier.create(comp.compactIfNeeded(hist))
            .assertNext(h -> {
                ...
                boolean hasSummary = h.all().stream()
                    .anyMatch(m -> m instanceof Message.System s && s.content().contains("[SUMMARY]"));
