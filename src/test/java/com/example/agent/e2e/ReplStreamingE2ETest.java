...
class ReplStreamingE2ETest extends E2ETestBase {
    ...
    void streamsHelloResponse() throws Exception {
        ...
        StepVerifier.create(provider.streamChat(new com.example.agent.provider.ChatRequest(
                "deepseek-chat", null, java.util.List.of(), java.util.List.of(),
                1.0, 1000, java.util.Map.of())).collectList())
            .assertNext(chunks -> {
                boolean hasText = chunks.stream().anyMatch(c ->
                    c instanceof StreamChunk.TextDelta t && t.text().contains("你好"));
                boolean hasUsage = chunks.stream().anyMatch(c ->
                    (c instanceof StreamChunk.Finished f && f.usage() != null && f.usage().promptTokens() == 10)
                    || (c instanceof StreamChunk.Usage u && u.promptTokens() == 10));
        ...
        wireMock.verify(WireMock.postRequestedFor(urlEqualTo("/v1/chat/completions"))
