...
class ChatCommandFriendlyErrorTest {
    ...
    void unauthorizedMapsToApiKeyHint() {
        Throwable e = WebClientResponseException.create(401, "Unauthorized", null, null, null);
        String out = ChatCommand.friendlyError(e);
        assertTrue(out.contains("401"), "should mention status code: " + out);
        assertTrue(out.contains("DEEPSEEK_API_KEY"), "should mention DEEPSEEK_API_KEY: " + out);
    }

    @Test
    void notFoundMapsToBaseUrlHint() {
        Throwable e = WebClientResponseException.create(404, "Not Found", null, null, null);
        String out = ChatCommand.friendlyError(e);
        assertTrue(out.contains("404"), "should mention status code: " + out);
        assertTrue(
                out.contains("baseUrl") || out.contains("model"),
                "should mention baseUrl/model: " + out);
    }

    @Test
    void rateLimitMapsToWaitHint() {
        Throwable e = WebClientResponseException.create(429, "Too Many Requests", null, null, null);
        String out = ChatCommand.friendlyError(e);
    ...
    void connectExceptionMapsToNetworkHint() {
        ...
        String out = ChatCommand.friendlyError(e);
        assertTrue(
                out.contains("网络") || out.toLowerCase().contains("network"),
