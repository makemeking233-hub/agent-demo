...
public class ShellTool implements Tool<ShellTool.Input, String> {
    ...
    /**
     * 显式参数的 {@link ThreadPoolExecutor}（规范 10.1-1：禁止 Executors）。
     * 单线程 + 有界队列（避免 OOM，规范 10.1-2）。
     */
    private static ExecutorService createIoPool() {
        return new ThreadPoolExecutor(
            POOL_CORE_SIZE, POOL_CORE_SIZE,
            POOL_KEEP_ALIVE_SEC, TimeUnit.SECONDS,
    ...
    /**
     * 追加字节到 buf，超过 maxOutputBytes 时追加截断标记并返回 true（表示停止读取）。
     * 提取为独立方法以降低 readBounded 的嵌套层级（规范 14 嵌套 ≤ 4）。
     */
    ...
    private String safeGet(Future<String> f) {
        try { return f.get(FUTURE_WAIT_SEC, TimeUnit.SECONDS); } catch (Exception e) {
