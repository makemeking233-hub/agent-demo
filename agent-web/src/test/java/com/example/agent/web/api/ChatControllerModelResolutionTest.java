...
class ChatControllerModelResolutionTest {
    ...
    /** 捕获 streams.create(...) 实际收到的 model（add-provider-catalog-abstract 起为第 3 个参数） */
    private final AtomicReference<String> capturedModel = new AtomicReference<>();
    /** 捕获 streams.create(...) 实际收到的 providerId（第 1 个参数） */
    ...
    void setUp() {
        ...
        // add-provider-catalog-abstract：ChatController 改调 6 参重载
        // create(providerId, sessionId, model, mode, workspace, reasoningEffort)
    ...
    @Test
    void inferredProviderIsPassedToStreamService() {
        // add-provider-catalog-abstract task 7.3：合法 model → 按前缀推断 provider 并透传
        send("deepseek-reasoner");
        assertThat(capturedProviderId.get()).isEqualTo("deepseek");
    }
    ...
    void invalidModelDoesNotCreateAnyStream() {
        send("deepseek-chat");
        verify(streams, never()).create(any(), any(), any(), any(), any(), any());
